/**
  ******************************************************************************
  * @file    LwIP/LwIP_HTTP_Server_Netconn_RTOS/LWIP/App/httpserver-netconn.c
  ...
  if (recv_err == ERR_OK)
  {
    if (netconn_err(conn) == ERR_OK)
    {
      ...
      if ((buflen >=5) && (strncmp(buf, "GET /", 5) == 0))
      {
        ...
        if (strncmp((char const *)buf,"GET /STM32H5xx_files/ST.gif",27)==0)
        {
          fs_open(&file, "/STM32H5xx_files/ST.gif");
          netconn_write(conn, (const unsigned char*)(file.data), (size_t)file.len, NETCONN_NOCOPY);
          fs_close(&file);
        }
                /* Check if request to get stm32.png */
        else if (strncmp((char const *)buf,"GET /STM32H5xx_files/stm32.png",30)==0)
        {
          fs_open(&file, "/STM32H5xx_files/stm32.png");
          netconn_write(conn, (const unsigned char*)(file.data), (size_t)file.len, NETCONN_NOCOPY);
          fs_close(&file);
        }
        else if (strncmp((char const *)buf,"GET /STM32H5xx_files/logo.jpg", 29) == 0)
        {
        ...
        else if(strncmp(buf, "GET /STM32H5xxTASKS.html", 24) == 0)
        {
           /* Load dynamic page */
           DynWebPage(conn);
        }
        else if((strncmp(buf, "GET /STM32H5xx.html", 19) == 0)||(strncmp(buf, "GET / ", 6) == 0))
        {
          /* Load STM32H7xx page */
...
{
  sys_thread_new("HTTP", http_server_netconn_thread, NULL, 2048, WEBSERVER_THREAD_PRIO);
}
